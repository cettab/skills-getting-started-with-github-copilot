      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length === 0) {
          ...
          details.participants.forEach(email => {
            participantsHTML += `<li class="participant-item">${email} <span class="delete-participant" title="Remove participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}">&#128465;</span></li>`;
          });
        }
        participantsHTML += "</ul>";
        ...
        activityCard.querySelectorAll('.delete-participant').forEach(icon => {
          icon.addEventListener('click', async (e) => {
            ...
            try {
              const resp = await fetch(`/activities/${encodeURIComponent(activityName)}/participant?email=${encodeURIComponent(email)}`, {
                method: 'DELETE',
              });
